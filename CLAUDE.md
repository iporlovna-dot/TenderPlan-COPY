...
Тумблер над лентой: человек один раз грузит своё ТЗ, лента показывает **% совпадения
по каждой закупке**, сортируется по нему и объясняет в карточке, что совпало, а чего нет.
...
- В снапшот кладём **термины, а не текст**: топ-`LK_TZ_MAX_TERMS` (120) по длине —
  длина груба, но отсекает воду и оставляет числовые требования и специфичные слова.
...
- Своё ТЗ живёт в localStorage (`lekalo_my_tz`) **терминами, а не текстом** — там лимит
  ~5 МБ на домен, и его делят доска с поисками.
...
Env: `LK_TZ_DOCS` (бюджет документов за прогон, 120), `LK_TZ_MAX_TERMS` (120),
