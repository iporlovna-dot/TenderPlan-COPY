...
# Порог косинусного сходства имён+значений для эмбеддинг-маппинга (plan.md §3.7). ВЫСОКИЙ и
# precision-first по итогу калибровки на bge-m3 (2026-07-28): при 0.85 — 0 ложных пар (чужой
# «материал_корпуса↔материал_световода» = 0.63 отсекается), ясные синонимы (материал 0.96,
# разрешение 0.95, тип 0.92) берутся. Ниже порога — НЕ маппим: пробел безопаснее ложной пары
# (§3.6 «неверный маппинг хуже отсутствующего»). Трудные синонимы добирает словарь key_synonyms.
_EMBED_THRESHOLD = float(os.getenv("SPECMATCH_EMBED_THRESHOLD", "0.85"))
