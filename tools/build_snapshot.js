...
// 2 (2026-08-25): .xlsx перестал быть безнадёжным (isHopeless) и получил вес в
// rankTzDocs — записи, помеченные «формат не тот» на Excel-ТЗ (частых у 223-ФЗ),
// надо переоткрыть и разобрать. Успешные .docx-разборы не трогаются.
const TZ_PICK = 2;
...
// 3 (2026-08-24): товарные таблицы без КТРУ (pickGoodsTable), пропуск строки-
// нумерации, комбинированная шапка «Наименование товара/…/количество» — переразбор
// удачных разборов без спецификации, чтобы достать реальные названия вместо стемов.
// 4 (2026-08-25): поиск шапки в первых непустых строках (не только [0,1,2] —
// у Excel-ТЗ 223-ФЗ титул+пустые строки сверху), «объём» как количество, колонка
// свободных требований (reqText) → характеристика позиции. Затрагивает и .docx.
// 5 (2026-08-31): раскрытие w:gridSpan в docxTables — объединённая шапка
// «Характеристики товара» (charName/charValue/ед.изм/инструкция одной ячейкой)
// давала съехавшие относительно строк данных колонки, charValue терялась
// целиком, таблица при этом находилась (status "ok", items без chars). Найдено
