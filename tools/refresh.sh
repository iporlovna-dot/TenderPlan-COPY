...
if [ "$before" = "$after" ]; then
  ...
  # Лента и довески — пять файлов, и они обязаны быть ОДНОЙ версии: свежая
  ...
  # tztext.json браузер не грузит (см. build_snapshot.js), но версия обязана
  # совпадать с spec.json: бэкенд читает оба по одному purchase_id разом.
  DATA_FILES="purchases.json tz.json docs.json spec.json tztext.json"
  ok=1
